classdef scalpmap
    ...
    methods
        ...
        function obj = normalizePolarity(obj, normalizationMethod) % normalize polarities
            if nargin < 2
                normalizationMethod = 'monte-carlo';
            end;
        ...
        function newObj = createSubsetForId(obj, subsetId, renormalizeScalpmapPolarity)
            ...
            if ~isempty(newObj.numberInDataset)
            ...
            for i=1:length(subsetId)
                ...
                try
                    if ~isempty(obj.subjectName) && ~isempty(obj.numberInDataset)
                        titleText = [num2str(subsetId(i)) '-' obj.subjectName{subsetId(i)} ' / IC' num2str(obj.numberInDataset(subsetId(i)))];
                        title(titleText);
                    else
        ...
        function obj = addFromChannels(obj, channelWeight, chanlocs, numberInDataset, subjectName, varargin)
            % obj = addFromChannels(obj, channelWeight, chanlocs, numberInDataset, subjectName, varargin)
